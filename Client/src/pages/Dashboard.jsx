...
// // PDF parsing in the browser using pdfjs
...
//       // ai upload returns { resumeId }
//       const resumeId = data.resume?._id || data.resumeId;
//       if (!resumeId) throw new Error('Server did not return a resume id');
...
//       // server may return { resume } or { resumeId }
//       const uploadedId = data.resume?._id || data.resumeId;
//       if (!uploadedId) {
//         console.error('Unexpected upload response', data);
//         throw new Error('Server did not return resume id after upload');
//       }
...
//       console.log('Deleting resume:', resumeId, 'token present?', !!token);
...
//       toast.success(data.message);
//     } catch (error) {
//       console.error('Delete resume error response:', error.response?.data, error.message);
//       toast.error(error.response?.data?.message || error.message);
//     }
//   }
...
//   return (
//     <div>
//       <div className="min-w-7xl mx-auto px-4 py-8">       
          ...
          {allResumes.map((resume, index) => {
            ...
            return (
              ...
              >
                ...
                <p
                  className="absolute bottom-1 text-[11px]text-slate-400 group-hover:text-slate-500transition-all duration-300 px-2 text-center"
                  style={{ color: baseColor + "90" }}
                >
